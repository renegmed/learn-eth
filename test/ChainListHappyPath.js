...
contract('ChainList', function(accounts){
    ...
    it("should be initialized with empty values", function(){
        return ChainList.deployed().then(function(instance){
            ...
            return chainListInstance.getArticlesForSale();  // returns array uint[]
        }).then(function(data){
    ...
    it("should let us sell a first article", function() {
        return ChainList.deployed().then(function(instance){
            chainListInstance = instance;
            return chainListInstance.sellArticle(  // returns LogSellArticle event
                articleName1, 
                articleDescription1, 
                web3.toWei(articlePrice1,"ether"), {from: seller}
            );        
        }).then(function(receipt){  // LogSellArticle event which has logs[] array of length 1
            ...
            assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei(articlePrice1,"ether"), "event article price must " + web3.toWei(articlePrice1,"ether"));
            return chainListInstance.getNumberOfArticles();
        }).then(function(data){
        ...
        }).then(function(data){ // receives array based on Article struct
            assert.equal(data[0].toNumber(),1, "articel id must be 1");
    ...
    it("should let us sell a second article", function(){
        return ChainList.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei(articlePrice2,"ether"), "event second article price must " + web3.toWei(articlePrice2,"ether"));
            return chainListInstance.getNumberOfArticles();
        }).then(function(data){  // receives uint
            ...
            assert.equal(data[0].toNumber(),2, "articel id must be 2");
            assert.equal(data[1], seller, "seller must be " + seller);
            assert.equal(data[2], 0x0, "buyer must be empty");
    ...
    it("should buy an article", function(){
        ...
        }).then(function(receipt){  // receives an event with logs[] array as a member
            assert.equal(receipt.logs.length, 1, "one event should have been triggered");
        ...
        }).then(function(data){ // receives uint[] of article IDs
